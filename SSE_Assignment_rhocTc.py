...
#add theoretical ms track of ms star using solar metallicity
# from old script
...
def T_core(M, rho_c):
    rhoc = np.log10((0.42*const.G.cgs.value/Ke)**3*mue**5*M**(2.))
    if any(LB < rho_c < UB for rho_c, LB, UB in zip(rho_c, [-10]*len(rho_c), [rhoc]*len(rho_c))):
    	T = 0.42*const.G.cgs.value/const.R.cgs.value*mu*M**(2./3.)*rho_c**(1./3.) #Bn * (4.*np.pi/3.)**(1./3.) * const.G.cgs.value * M**(2./3.) * mu*const.u.cgs.value/const.M_sun.cgs.value * rho_c**(1./3.)
    	plt.plot(np.log10(rho_c), np.log10(T), c="black", label="Theoretical evolution of " + str(M/const.M_sun.cgs.value) + " $M_\odot$")
    plt.plot([[rhoc]]*100, np.linspace(np.log10(T[-1]),2,100),linestyle = ':', c="black")
    #plt.vlines(rhoc, 3, np.log10(T[-1]))
    
Ms = np.array([1., 2.])*const.M_sun.cgs.value

for M in range(len(Ms)):
    T_core(Ms[M], rho) #, c="black", label="Theoretical evolutionary tracks")
    


# single star evolution tracks
plt.scatter(rho_1,Teff_1, edgecolors=age1_colors, color='white', linewidths=0.5,zorder=100,label = r'$1M_\odot$ star')
plt.scatter(rho_2,Teff_2, edgecolors=age2_colors, color='black', linewidths=0.5,zorder=100, label = r'$2M_\odot$ star')
...
plt.plot(np.log10(1.622e2 ),np.log10(1.571e7), marker = '*',markersize = '15', label = r'Sun',zorder = 101, color = 'orange')
...
plt.plot(np.log10(rho), np.log10(T1), color="saddlebrown", alpha=0.8,lw=3, label=r"$P_{id}=P_{rad}$")
plt.plot(np.log10(rho[rho<rho4]), np.log10(T2[rho<rho4]), color="darkgreen",alpha=0.8, lw=3, label=r"$P_{id}=P_{NR,e}^D$")
plt.plot(np.log10(rho[rho>rho4]), np.log10(T2[rho>rho4]), color="darkgreen",alpha=0.8, ls=":")
plt.plot(np.log10(rho[rho<rho4]), np.log10(T3[rho<rho4]), color="darkmagenta",alpha=0.8, ls=":")
plt.plot(np.log10(rho[rho>rho4]), np.log10(T3[rho>rho4]), color="darkmagenta",alpha=0.8, lw=3, label=r"$P_{id}=P_{ER,e}^D$")
plt.plot(np.log10(len(T2[T2<T3])*[rho4]), np.log10(T2[T2<T3]), color="black",alpha=0.8, lw=3, label=r"$P_{NR,e}^D=P_{ER,e}^D$")
plt.plot(np.log10(rho), np.log10(T4), ls="-.", color="lightgreen",alpha=0.8, label=r"$P_{rad}=P_{NR,e}^D$")
plt.plot(np.log10(rho), np.log10(T5), ls="--", color="peachpuff",alpha=0.8, label=r"$P_{rad}=P_{ER,e}^D$")
...
plt.text(7.2,4, 'Relativistic \n degenerate gas', size = 15)

plt.ylim(3,11)
plt.xlim(-4,10)
plt.xlabel(r'log10($\rho_c$) $[g/cm^3]$',size=15)
